...
class Runner(object):
    ...
    def __init__(
        ...
        if num_parallel - int(self.evaluation) > 1:
            ...
            self.agent_order = Agent.create(agent=agent, environment=environment)
            self.agent_machine1 = Agent.create(agent=agent, environment=environment)
            self.agent_machine2 = Agent.create(agent=agent, environment=environment)
            self.agent_machine3 = Agent.create(agent=agent, environment=environment)
            self.agent_machine4 = Agent.create(agent=agent, environment=environment)
    ...
    def run(
        ...
        # Evaluation
        evaluation=False, save_best_agent=None, save_best_agent_order=None, save_best_agent_machine=None, evaluation_callback=None
    ):
        """
        Run experiment.
        ...
        """
        #agent = self.agent.load(directory='D:\SimRL_onward')
        #agent = self.agent.restore(directory='D:\\SimRL_onward\\models',filename='runner_test')

        #agent_failure = self.agent.load(directory='D:\\SimRL_onward\\model', filename='runner_failure')
        #agent_bal = self.agent.load(directory='D:\\SimRL_onward\\model', filename='runner_bal')
        #print("agent restore: ",agent_failure)
        #print("agent restore: ", agent_bal)
        ...
        self.episode_rewards_order = list()
        self.episode_rewards_machine1 = list()
        self.episode_rewards_machine2 = list()
        self.episode_rewards_machine3 = list()
        self.episode_rewards_machine4 = list()
        ...
        if self.evaluation or evaluation:
            ...
            self.evaluation_rewards_order = list()
            self.evaluation_rewards_machine1 = list()
            self.evaluation_rewards_machine2 = list()
            self.evaluation_rewards_machine3 = list()
            self.evaluation_rewards_machine4 = list()
            ...
            if len(self.environments) == 1:
                ...
                self.episode_rewards_order = self.evaluation_rewards_order
                self.episode_rewards_machine1 = self.evaluation_rewards_machine1
                self.episode_rewards_machine2 = self.evaluation_rewards_machine2
                self.episode_rewards_machine3 = self.evaluation_rewards_machine3
                self.episode_rewards_machine4 = self.evaluation_rewards_machine4
            ...
            self.evaluation_rewards_order = self.episode_rewards_order
            self.evaluation_rewards_machine1 = self.episode_rewards_machine1
            self.evaluation_rewards_machine2 = self.episode_rewards_machine2
            self.evaluation_rewards_machine3 = self.episode_rewards_machine3
            self.evaluation_rewards_machine4 = self.episode_rewards_machine4
        ...
        self.episodes = 0
        self.updates = 0
        self.updates_order = 0
        self.updates_machine = 0
        ...
        self.save_best_agent_order = save_best_agent_order
        self.save_best_agent_machine1 = save_best_agent_machine
        self.save_best_agent_machine2 = save_best_agent_machine
        self.save_best_agent_machine3 = save_best_agent_machine
        self.save_best_agent_machine4 = save_best_agent_machine
        ...
        # Episode statistics
        self.episode_rewards = [0.0 for _ in self.environments]
        self.episode_rewards_order = [0.0 for _ in self.environments]
        self.episode_rewards_machine1 = [0.0 for _ in self.environments]
        self.episode_rewards_machine2 = [0.0 for _ in self.environments]
        self.episode_rewards_machine3 = [0.0 for _ in self.environments]
        self.episode_rewards_machine4 = [0.0 for _ in self.environments]
        ...
        self.rewards_order = [None for _ in self.environments]
        self.rewards_machine = [None for _ in self.environments]
        if self.evaluation_run:
            self.evaluation_internals = self.agent.initial_internals()

        # Required if agent was previously stopped mid-episode
        self.agent.reset()
        self.agent_order.reset()
        self.agent_machine1.reset()
        self.agent_machine2.reset()
        self.agent_machine3.reset()
        self.agent_machine4.reset()
        ...
        while any(terminal <= 0 for terminal in self.prev_terminals):
                ...
                while any(terminal is None for terminal in self.terminals):
                    for n in range(len(self.environments)):
                        if self.terminals[n] is not None:
                            ...
                            self.rewards[n] = None
                            self.rewards_order[n] = None
                            self.rewards_machine[n] = None
            ...
            for n in range(len(self.environments)):
                ...
                if not self.batch_agent_calls:
                    #print("15")
                    #print("self._expect_receive: ", self.environments[n]._expect_receive)
                    self.states[n], self.terminals[n], self.rewards[n] = observation
    ...
    def handle_act(self, parallel):
        if self.batch_agent_calls:
            ...
            actions_list_order = []
            actions1 = self.agent.act(states=self.states[0][0:91][parallel], parallel=parallel)
            #print("action1: ", actions1)
            actions2 = self.agent_order.act(states=self.states[0][91:132][parallel], parallel=parallel)
            #print("action2: ", actions2)
            actions3.append(self.agent_machine1.act(states=self.states[0][132:138][parallel], parallel=parallel))
            actions3.append(self.agent_machine2.act(states=self.states[0][138:144][parallel], parallel=parallel))
            actions3.append(self.agent_machine3.act(states=self.states[0][144:150][parallel], parallel=parallel))
            actions3.append(self.agent_machine4.act(states=self.states[0][150:156][parallel], parallel=parallel))
            #print("action3: ", actions3)
            while len(actions_list_order)<5:
                actions2 = self.agent_order.act(states=self.states[0][91:132][parallel], parallel=parallel, independent=True)
                if not actions_list_order.__contains__(actions2):
                    actions_list_order.append(actions2)
            #print("actions_list_order ",actions_list_order)
            actions = [actions1,actions_list_order,actions3]
            #print("actions   ::: " , actions)
            self.environments[parallel].start_execute(actions=actions)
            self.episode_agent_second[parallel] += time.time() - agent_start
            #Voting between actions

            #c = Counter([actions,actions_1,actions_2])
            #print("majority: ",c)
            #value, count = c.most_common()[0]
            #if count > 1:
                #self.environments[parallel].start_execute(actions=value)
            #else:
                #random select actions
                #print("action1: ", actions_1)
                #print("action2: ", actions_2)
                #print("action: ", actions)
                #selected_action = random.choices([actions, actions_1, actions_2],weights=(1, 1, 1), k=1)[0]
    ...
    def handle_act_joint(self, runner_type):
        parallel = [
        ...
        if len(parallel) > 0:
            ...
            states=[self.states[0:91][p] for p in parallel], parallel=parallel
            ),  self.agent_order.act(
            states=[self.states[91:132][p] for p in parallel], parallel=parallel
            ),  self.agent_order.act(
            states=[self.states[132:][p] for p in parallel], parallel=parallel
            )]
            ...
            self.actions = [[
            self.actions[0][parallel.index(n)] if n in parallel else None
            for n in range(len(self.environments))
            ], [
            self.actions[1][parallel.index(n)] if n in parallel else None
            for n in range(len(self.environments))
            ], [
            self.actions[2][parallel.index(n)] if n in parallel else None
            for n in range(len(self.environments))
            ]]

        if self.evaluation_run and self.terminals[-1] <= 0:
            agent_start = time.time()
            actions1, evaluation_internals1 = self.agent.act(
            states=self.states[0:91][-1], internals=self.evaluation_internals, evaluation=True
            )
            actions2, evaluation_internals2 = self.agent_order.act(
            states=self.states[91:132][-1], internals=self.evaluation_internals, evaluation=True
            )
            actions31, evaluation_internals31 = self.agent_machine1.act(
                states=self.states[132:138][-1], internals=self.evaluation_internals, evaluation=True
            )
            actions32, evaluation_internals32 = self.agent_machine2.act(
                states=self.states[138:144][-1], internals=self.evaluation_internals, evaluation=True
            )
            actions33, evaluation_internals33 = self.agent_machine3.act(
                states=self.states[144:150][-1], internals=self.evaluation_internals, evaluation=True
            )
            actions34, evaluation_internals34 = self.agent_machine4.act(
                states=self.states[150:156][-1], internals=self.evaluation_internals, evaluation=True
            )
            self.actions[-1], self.evaluation_internals = [actions1,actions2,[actions31,actions32,actions33,actions34]],[evaluation_internals1,evaluation_internals2,evaluation_internals3]
    ...
    def handle_act_evaluation(self):
        if self.batch_agent_calls:
            ...
            actions , self.evaluation_internals = [actions1,actions2,[actions31,actions32,actions33,actions34]],[evaluation_internals1,evaluation_internals2,evaluation_internals3]
    ...
    def handle_observe(self, parallel):
        # Update episode statistics
        reward1 = self.rewards[0][parallel]
        reward2 = self.rewards[parallel][1]
        reward3 = self.rewards[parallel][2]
        #print("reward3:     ", reward3[0])
        self.episode_rewards[parallel] += reward1
        self.episode_rewards_order[parallel] += reward2
        self.episode_rewards_machine1[parallel] += reward3[0]
        self.episode_rewards_machine2[parallel] += reward3[1]
        self.episode_rewards_machine3[parallel] += reward3[2]
        self.episode_rewards_machine4[parallel] += reward3[3]
        ...
        if not self.batch_agent_calls:
            ...
            terminal=self.terminals[parallel], reward=self.rewards[0][parallel], parallel=parallel
            )
            self.updates += int(updated)
            updated = self.agent_order.observe(
                terminal=self.terminals[parallel], reward=self.rewards[parallel][1], parallel=parallel
            )
            self.updates += int(updated)
            updated = self.agent_machine1.observe(
                terminal=self.terminals[parallel], reward=self.rewards[parallel][2][0], parallel=parallel
            )
            self.updates += int(updated)
            updated = self.agent_machine2.observe(
                terminal=self.terminals[parallel], reward=self.rewards[parallel][2][1], parallel=parallel
            )
            self.updates += int(updated)
            updated = self.agent_machine3.observe(
                terminal=self.terminals[parallel], reward=self.rewards[parallel][2][2], parallel=parallel
            )
            self.updates += int(updated)
            updated = self.agent_machine4.observe(
                terminal=self.terminals[parallel], reward=self.rewards[parallel][2][3], parallel=parallel
            )
            self.updates += int(updated)
            if self.updates >= self.num_updates * 2:
                self.terminate = 2
            self.episode_agent_second[parallel] += time.time() - agent_start

        # Maximum number of updates (after counter increment!)

    def handle_observe_joint(self,runner_type):
        parallel = [
            n for n in range(len(self.environments) - int(self.evaluation_run))
        ...
        if len(parallel) > 0:
            ...
            reward=[self.rewards[0][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            updated = self.agent_order.observe(
            terminal=[self.terminals[p] for p in parallel],
            reward=[self.rewards[1][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            updated = self.agent_machine1.observe(
                terminal=[self.terminals[p] for p in parallel],
                reward=[self.rewards[2][0][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            updated = self.agent_machine2.observe(
                terminal=[self.terminals[p] for p in parallel],
                reward=[self.rewards[2][1][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            updated = self.agent_machine3.observe(
                terminal=[self.terminals[p] for p in parallel],
                reward=[self.rewards[2][2][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            updated = self.agent_machine4.observe(
                terminal=[self.terminals[p] for p in parallel],
                reward=[self.rewards[2][3][p] for p in parallel], parallel=parallel
            )
            self.updates += updated
            agent_second = (time.time() - agent_start) / len(parallel)
            for p in parallel:
    ...
    def handle_observe_evaluation(self):
        # Update episode statistics
        self.episode_rewards[-1] += self.rewards[0][-1]
        self.episode_rewards_order[-1] += self.rewards[1][-1]
        self.episode_rewards_machine[-1] += self.rewards[2][-1]
    ...
    def handle_terminal(self, parallel):
        ...
        self.episode_rewards.append(self.episode_rewards[parallel])
        self.episode_rewards_order.append(self.episode_rewards_order[parallel])
        self.episode_rewards_machine1.append(self.episode_rewards_machine1[parallel])
        self.episode_rewards_machine2.append(self.episode_rewards_machine2[parallel])
        self.episode_rewards_machine3.append(self.episode_rewards_machine3[parallel])
        self.episode_rewards_machine4.append(self.episode_rewards_machine4[parallel])
        ...
        self.episode_rewards[parallel] = 0.0
        self.episode_rewards_order[parallel] = 0.0
        self.episode_rewards_machine1[parallel] = 0.0
        self.episode_rewards_machine2[parallel] = 0.0
        self.episode_rewards_machine3[parallel] = 0.0
        self.episode_rewards_machine4[parallel] = 0.0
    ...
    def handle_terminal_evaluation(self):
        ...
        if self.save_best_agent_order is not None:
            evaluation_score = self.evaluation_callback(self)
            assert isinstance(evaluation_score, float)
            if self.best_evaluation_score is None:
                self.best_evaluation_score = evaluation_score
                ...
                self.best_evaluation_score = evaluation_score
                self.agent_order.save(
                    directory=self.save_best_agent_order, filename='best-model', append_timestep=False
                )
        else:
            self.evaluation_callback(self)
        if self.save_best_agent_machine is not None:
            evaluation_score = self.evaluation_callback(self)
            assert isinstance(evaluation_score, float)
            if self.best_evaluation_score is None:
                self.best_evaluation_score = evaluation_score
                ...
                self.best_evaluation_score = evaluation_score
                self.agent_machine1.save(
                    directory=self.save_best_agent_machine1, filename='best-model', append_timestep=False
                )
                self.agent_machine2.save(
                    directory=self.save_best_agent_machine2, filename='best-model', append_timestep=False
                )
                self.agent_machine3.save(
                    directory=self.save_best_agent_machine3, filename='best-model', append_timestep=False
                )
                self.agent_machine4.save(
                    directory=self.save_best_agent_machine4, filename='best-model', append_timestep=False
                )
        else:
            self.evaluation_callback(self)
        # Maximum number of episodes or episode callback (after counter increment!)
