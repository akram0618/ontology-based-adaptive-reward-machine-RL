...
class TensorforceWorker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory ):
        ...
        for n in range(round(budget)):
            ...
            #we add run.py code here
            runner = Runner(agent='D:\SimRL_onward\config\/trpo1.json', environment=self.environment)
            runner.run(num_episodes=NUM_EPISODES, use_tqdm=False)
    ...
    def get_configspace():
        ...
        batch_size = cs.hyperparameters.UniformIntegerHyperparameter(
            name='batch_size', lower=1, upper=2, log=True
        )
...
def main():
    ...
    args.environment = 'production.envs.ProductionEnv'
    print("args: ", args)
