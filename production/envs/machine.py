...
class Machine(Resource):
    ...
    def __init__(self, env, id, capacity, agent_type, machine_group, statistics, parameters, resources, agents,
        ...
        self.process = self.env.process(self.processing('test'))  # Process started at creation
    ...
    def is_free_machine_group(self):
        #print("machine.is_free_machine_group")
    ...
    def get_max_waiting_time(self):
        #print("machine.get_max_waiting_time")
        max_wt = None
    ...
    def get_utilization_step(self):
        #print("machine.get_utilization_step")
    ...
    def reactivate_transport_if_idle(self):
        #print("machine.reactivate_transport_if_idle")
    ...
    def put_buffer_in(self, order):
        ...
        if self.idle.triggered:
            ...
            self.process = self.env.process(self.processing(test='test'))  # Restart idle process

    def get_buffer_in(self, order):
        #print("machine.get_buffer_in")
        self.statistics['stat_inv_buffer_in_mean'][1][self.id] = (self.statistics['stat_inv_buffer_in_mean'][1][
                                                                      self.id] *
                                                                  self.statistics['stat_inv_buffer_in_mean'][0][
                                                                      self.id] +
    ...
    def get_buffer_in_by_index(self, id):
        #print("machine.get_buffer_in_by_index")
        self.statistics['stat_inv_buffer_in_mean'][1][self.id] = (self.statistics['stat_inv_buffer_in_mean'][1][
                                                                      self.id] *
                                                                  self.statistics['stat_inv_buffer_in_mean'][0][
                                                                      self.id] +
    ...
    def is_free_buffer_out(self):
        #print("machine.is_free_buffer_out")
        currently_processing = 0
        if self.buffer_processing != None:
            currently_processing = 1
        return len(self.buffer_in) + len(self.buffer_out) + currently_processing <= self.capacity
    ...
    def get_next_action(self):
        ...
        if action < len(self.buffer_in):
            result_order = self.get_buffer_in(self.buffer_in[action])
        elif len(self.buffer_in) > 0:
            result_order = self.get_buffer_in(self.buffer_in[0])
        else:
            result_order = self.get_buffer_in(None)
        #print("result order: " , result_order , "  waiting time:  " , result_order.get_total_waiting_time())
        if self.parameters['PRINT_CONSOLE']: print("MACHINE-Action: ActionID ", action[0].id, " - Reward: ", reward)

        return result_order

    def calculate_state(self):
        states = [0.0] * self.capacity
        id = 0
        if len(self.buffer_in) > 0:
            for order in self.buffer_in:
                mach_ord_buffer_in_wt = order.get_total_waiting_time()
                states[id] = mach_ord_buffer_in_wt/100
                #mach_ord_buffer_in_priority = order.order_priority
                #states[id] = mach_ord_buffer_in_priority
                id += 1
        #print("machine orders states2: ", states)
        return states

    def processing(self, test):
    ...
    def actions(self):
        return dict(type='int', shape=(40))

    def break_machine(self):
        #print("machine.break_machine")
    ...
    def calculate_reward(self,action):
        #result_reward = get_reward_machine_order_priority(self, action)
        result_reward = get_reward_machine_order_waiting_time(self, action)
        self.latest_reward = result_reward
        return result_reward

def other_jobs(env, repairman):
    #print("machine.other_jobs")
