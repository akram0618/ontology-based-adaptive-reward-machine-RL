...
def get_reward_utilization(transport_resource, invalid_reward):
    ...
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        ...
        des = transport_resource.next_action_destination
        #if des.type == 'machine':
            #print("buffer: ",des.capacity - len(des.buffer_in))
            #print("MTBF: ", des.parameters['MTBF'][des.id])
        util = 0.0
    ...
    #print("result_reward_util: ",result_reward)
    return result_reward

def get_reward_utilization_2(transport_resource, invalid_reward):
    result_reward = invalid_reward
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_WAITING_ACTION']
    elif transport_resource.next_action_valid:
        ...
        if des.type == 'machine':
            #print("buffer: ", des.capacity - len(des.buffer_in))
            #print("MTBF: ", des.parameters['MTBF'][des.id])
            failure_reward = 0.01 * des.parameters['MTBF'][des.id]
            #print("failure:  ", failure_reward)
            buffer_reward = 6 / (1 + len(des.buffer_in)+ len(des.buffer_out))
            #print("buffer:  ", buffer_reward)
        else:
            failure_reward = 0.0
            buffer_reward = 0.0
        util = 0.0
        ...
        util = util / transport_resource.parameters['NUM_MACHINES']
        #print("util: ", util)
        transport_resource.last_reward_calc = util + failure_reward #+ buffer_reward

        result_reward = np.exp(util / 1.5) - 1.0 + failure_reward #+ buffer_reward
        if transport_resource.next_action_destination.type == 'machine':
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][0] * result_reward
        else:
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][1] * result_reward
...
def get_reward_duedate(transport_resource, invalid_reward, machine_orders):
    result_reward = invalid_reward
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_WAITING_ACTION']
    elif transport_resource.next_action_valid:
        ...
        var2 = 0.0
        for mach in transport_resource.resources['machines']:
            if(len(machine_orders[mach.id])>0):
                var = machine_orders[mach.id].pop()
                var2 = 100/(var[1]-var[2])
            util += var2
...
def get_reward_order_duedate(transport_resource, action):

    result_reward = 0.0
    if action > 0:
        if action < len(transport_resource.transp_orders):
            order = transport_resource.transp_orders[action]
            if order != -1:
                result_reward = 10000 / (order.order_waiting_threshold - order.get_total_waiting_time())
            else:
                result_reward = 0.0
        else:
            result_reward = 0.0
...
def get_reward_waiting_time(transport_resource, invalid_reward, machine_orders):
    result_reward = invalid_reward
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_WAITING_ACTION']
    elif transport_resource.next_action_valid:
        ...
        # We add order due date to reward calculation
        # result_reward += 0.001 * mach.parameters['MTBF'][mach.id] - 0.5
        if transport_resource.next_action_destination.type == 'machine':
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][0] * result_reward
        else:
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][1] * result_reward
...
def get_reward_order_waiting_time(transport_resource, action , invalid_reward):
    result_reward = invalid_reward
    result_reward = 0.0
    if len(action) > 0:
        if int(action[0]) < len(transport_resource.transp_orders):
            order = transport_resource.transp_orders[int(action[0])]
            if order != -1:
                result_reward = order.get_total_waiting_time()
            else:
                result_reward = 0.0
        else:
            result_reward = 0.0
...
def get_reward_order_priority(transport_resource, action ):

    result_reward = 0.0
    if action > 0:
        if int(action) < len(transport_resource.transp_orders):
            order = transport_resource.transp_orders[int(action)]
            if order != -1:
                result_reward = int(4 / order.order_priority)
            else:
                result_reward = 0.0
        else:
            result_reward = 0.0
...
def get_reward_machine_order_waiting_time(machine , action):
    #print("machine.buffer_in: ",machine.buffer_in , "action: ", action)
    if action < len(machine.buffer_in):
        order = machine.buffer_in[action]
        mach_ord_buffer_in_wt = order.get_total_waiting_time()
        result_reward = mach_ord_buffer_in_wt/10
    else:
        result_reward = 0
    return result_reward

def get_reward_machine_order_priority(machine , action):
    #print("machine.buffer_in: ",machine.buffer_in)
    #print("action: ", action)
    if action < len(machine.buffer_in):
        order = machine.buffer_in[action]
        mach_ord_buffer_in_priority = order.order_priority
        result_reward = 3 * (1/mach_ord_buffer_in_priority)
    else:
        result_reward = 0
    return result_reward

def get_reward_machine_failure(transport_resource, invalid_reward):
    result_reward = invalid_reward
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_WAITING_ACTION']
    elif transport_resource.next_action_valid:
        ...
        des = transport_resource.next_action_destination
        if des.type == 'machine':
            result_reward = 0.01 * des.parameters['MTBF'][des.id]
            #print("MTBF: ",des.parameters['MTBF'][des.id])
            # print("length of list:", len(machine_orders[mach.id]))
        else:
            result_reward = 0
        transport_resource.last_reward_calc = result_reward
        if transport_resource.next_action_destination.type == 'machine':
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][0] * result_reward
        else:
            result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_SUBSET_WEIGHTS'][1] * result_reward
...
def get_reward_machine_load_bal(transport_resource, invalid_reward):
    result_reward = invalid_reward
    if transport_resource.next_action_destination == -1 or transport_resource.next_action_origin == -1:  # Waiting or empty action selected
        result_reward = transport_resource.parameters['TRANSP_AGENT_REWARD_WAITING_ACTION']
    elif transport_resource.next_action_valid:
        ...
        des = transport_resource.next_action_destination
        if des.type == 'machine':
            result_reward = des.capacity - len(des.buffer_in)#+len(des.buffer_out)))/des.capacity
            #print("buffer: ", des.capacity - len(des.buffer_in))
            #result_reward = 60 /(1+len(des.buffer_in)+len(des.buffer_out))
        else:
            result_reward = 0
        transport_resource.last_reward_calc = result_reward
        if transport_resource.next_action_destination.type == 'machine':
