...
class ProductionEnv(Environment):
    ...
    def execute(self, actions):
        ...
        agent_order = self.resources['transps_ord'][0]
        agent_order_wt = self.resources['transps_ord'][1]
        agent_order_dd = self.resources['transps_ord'][2]
        agent_machine = self.resources['machines']
        ...
        for agent in Transport.agents_waiting_for_action:
            if int(actions[0]<40):
                ...
                if self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'direct':
                    agent.next_action = [int(actions[0])]
                    agent_order.next_action = actions[1]
                    agent_order_wt.next_action = actions[3]
                    agent_order_dd.next_action = actions[4]
                    for mach in agent_machine:
                        mach.next_action = 0
                    agent_machine[0].next_action = actions[2][0]
                    agent_machine[2].next_action = actions[2][1]
                    agent_machine[10].next_action = actions[2][2]
                    agent_machine[13].next_action = actions[2][3]
                elif self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'resource':
                    agent.next_action = [int(actions[0][0]), int(actions[0][1])]
                    agent_order.next_action = [actions[1]]
                    agent_order_wt.next_action = [actions[3]]
                    agent_order_dd.next_action = [actions[4]]
            ...
            reward1, terminal1 = agent.calculate_reward(actions[0])
            ...
            states1 = agent.calculate_state()  # Calculate state for next action determination

            if self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'direct':
                self.statistics['stat_agent_reward'][-1][3] = [int(actions[0])]
            elif self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'resource':
                self.statistics['stat_agent_reward'][-1][3] = [int(actions[0][0]), int(actions[0][1])]
            self.statistics['stat_agent_reward'][-1][4] = round(reward1, 5)
            self.statistics['stat_agent_reward'][-1][5] = agent.next_action_valid
            self.statistics['stat_agent_reward'].append([self.count_episode, self.counter, round(self.env.now, 5), None, None, None, states1])
        ...
        reward2 = []
        reward3 = []
        reward2 = agent_order.calculate_reward([actions[1],actions[3],actions[4]])
        states2 = agent_order.calculate_state()  # Calculate state for next action determination
        reward3.append(agent_machine[0].calculate_reward(actions[2][0]))
        reward3.append(agent_machine[2].calculate_reward(actions[2][1]))
        reward3.append(agent_machine[10].calculate_reward(actions[2][2]))
        reward3.append(agent_machine[13].calculate_reward(actions[2][3]))

        states3.extend(agent_machine[0].calculate_state())
        states3.extend(agent_machine[2].calculate_state())
        states3.extend(agent_machine[10].calculate_state())
        states3.extend(agent_machine[13].calculate_state())

        #print("reward3: ", reward3)
        states = states1
        reward = [reward1,reward2,reward3]
        states.extend(states2)
        states.extend(states3)
        #print("states: ", states)
        return states, terminal1, reward
    ...
    def reset(self):
        ...
        states1 = self.resources['transps'][0].calculate_state()
        states = states1
        states2 = self.resources['transps_ord'][0].calculate_state()
        states.extend(states2)
        states3 = self.resources['machines'][0].calculate_state()
        states.extend(states3)
        states3 = self.resources['machines'][2].calculate_state()
        states.extend(states3)
        states3 = self.resources['machines'][10].calculate_state()
        states.extend(states3)
        states3 = self.resources['machines'][13].calculate_state()
        states.extend(states3)
    ...
    def states(self):
        #print("production_env.states")
        state_type = 'bool'
        number = 0
        #if runner_type == 'machine':
        ...
        if 'total_process_time' in self.parameters['TRANSP_AGENT_STATE']:
            state_type = 'float'
            number += self.parameters['NUM_MACHINES']
        #elif runner_type == 'order':
        state_type = 'int'
        number2 = 40
        #print("State space size: ", number)
        return dict(type=state_type, num_values=number+number2 + int(self.parameters['MACHINE_CAPACITIES'][0]),shape = ())

    def actions(self):
        number = 0.0
        if self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'direct':
            number = len(self.resources['transps'][0].mapping)
            number2 = len(self.resources['transps'][0].mapping)
            #print("Action space size 1: ", number)
            #print("Action space size 2: ", number2)
            return dict(type='int', num_values=number2, shape = ())
        elif self.parameters['TRANSP_AGENT_ACTION_MAPPING'] == 'resource':
            shape = (2,)
            number = len(self.resources['transps'][0].mapping)
            number += len(self.resources['transps'][0].mapping)
            #print("Action space size: ", number)
            return dict(type='int', num_values=number, shape=())
