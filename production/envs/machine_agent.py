...
class Machine(Resource):
    ...
    def calculate_state(self):
        states = [-1] * self.capacity
        print("machine orders states1: ", states)
        if len(self.buffer_in) > 0:
            for order in self.buffer_in & id in len(self.buffer_in):
                mach_ord_buffer_in_wt = self.env.now - self.sop - self.time_handling
                states[id] = mach_ord_buffer_in_wt
